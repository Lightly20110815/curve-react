    ...
    <footer className="mt-section border-t border-rule-soft/60">
      ...
      <div className="container grid gap-10 py-10 md:grid-cols-[1.4fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-5 max-w-sm font-serif text-[15px] leading-[1.7] text-ink-body">
            ...
            <ul className="mt-4 space-y-2.5">
              {"links" in section && section.links.map((link) => (
                <li key={link.label}>
                  <Link
                    to={link.to}
                    className="font-serif text-[15px] text-ink-body transition-colors hover:text-stamp"
                  >
                    {link.label}
                  </Link>
                </li>
